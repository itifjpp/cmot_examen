		   ...
		   // _self.reset();
		    ...
		    for (var i = 0, f; f = files[i]; i++) {
		    	if (f.type.match('image.*') ) {
                            if(f.size>15172782){//Archivos mayores a 14.4 MB
                                    $(element).removeClass('loading done');
                                    msj_error_noti('El archivo seleccionado excede el tamaño pertimido, tamaño de archivo válido 15MB como máximo');
                                    continue;
                            }
		        }else{
