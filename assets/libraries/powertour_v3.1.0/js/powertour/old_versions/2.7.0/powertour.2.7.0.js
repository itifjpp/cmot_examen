	...
	Plugin.prototype = {
		...
		**/	
		init: function(){
			...
			**/	
			bd         = $('body');
			clickEvent = 'click touchstart';// up
		...
		**/
		_setPosition: function(s, i, ii, t, b){
			...
			**/	
			...
			var highLight = (s.highlight !== undefined && $.trim(s.highlight) != '') ? s.highlight : def.highlight;
		...
		**/
		_fxIn: function(csObj, fxIn){ 	
			...
			if(bd.attr(d_pwid) !== undefined){
				...
				$('.'+c_pwat).removeClass(c_pwat);
				
				csObj.parents('.'+c_pwhk).addClass(c_pwat);
		...
		_goTo: function(hook, step, center, position, scrollSpeed, scrollEasing, highlight, keepHighlighted, scrollHorizontal){
			...
			**/	
			function resetHighlight(){
				...
				if(highlight === false && keepHighlighted === false){
			
					$('#'+c_pwmk).fadeOut((isNaN(highlightEndSpeed)) ? 200 : highlightEndSpeed );
				}
			}
			resetHighlight();
		...
		_actionButtons: function(action, goto){ 
			...
			**/	
			...
			if(id !== undefined){
				...
				cs        = parseInt(cs);
				var allAc = $('[data-powertour-disable-all]').attr('data-powertour-disable-all');
				...
				**/	
				switch(action){
					case 'first':
						if(allAc === undefined && $('[data-powertour-disable-first]').attr('data-powertour-disable-first') === undefined){
							var newStep = 0;
						}else{
							return false;
						}
					break;
					case 'last':
						if(allAc === undefined && $('[data-powertour-disable-last]').attr('data-powertour-disable-last') === undefined){
							var newStep = total;
						}else{
							return false;
						}		
					break;
					case 'prev':
						if(allAc === undefined && $('[data-powertour-disable-prev]').attr('data-powertour-disable-prev') === undefined){
							if(cs <= 0){
								if(loopTour === true){
									var newStep = total;	
								}else{
					...
					case 'next':
					case 'timer':
						if(allAc === undefined && bd.attr('data-powertour-timer') != 'false' ||
						 $('[data-powertour-disable-next]').attr('data-powertour-disable-next') === undefined){
							if(cs >= total){
								if(loopTour === true){
									var newStep = 0;	
								}else{
					...
					break;
					case 'stop':
						if(allAc === undefined && $('[data-powertour-disable-stop]').attr('data-powertour-disable-stop') === undefined){
					...
					break;
					case 'goto':
						if(allAc === undefined && $('[data-powertour-disable-goto]').attr('data-powertour-disable-goto') === undefined){
							var newStep = goto - 1;	
						}else{
							return false;
						}		
