 ...
 * Version    : 2.0.0                                            *
 * Updated    : 2014-07-01 00:13:58 UTC+02:00                    *
 * Developer  : Mark                                             *
 * Dependency :                                                  *
 * Lib        : jQuery 1.7+                                      *
 * Licence    : NOT free                                         *
...
;(function($, window, document, undefined){
	...
	Plugin.prototype = {
		...
		**/
		_setPosition: function(s, i, ii, b){
			...
			**/			
			switch(position){
				// top left
				case 'tl':
					step.css({left: X , top: -stepHeight - offsetY});					
				break;
				// top middle
				case 'tm':
		...
		update: function(hook){ 
			...
			**/	
			$.each(hook, function(i, h){
				...
				* Setting the position.
				**/	
				self._setPosition(s, ti, si, false);	
			});
			...
			if(!isNaN(tour)){
				...
				**/	
				bd.attr({
					...
					'data-powertour-currentstep' : self.o.tours[tour].startWith,
					'data-powertour-timer'       : false 
				});
