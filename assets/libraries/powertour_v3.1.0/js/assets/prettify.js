...
$(document).ready(function($){
	...
	if (Array.prototype.indexOf) { // >IE8
		...
		$('.prettyprint[data-highlight-row]').each(function(i,e){
			$(this).children().children().each(function(i,e){
	
				var row = $(this).parents('.prettyprint').attr('data-highlight-row');

				if(row != undefined){
					
					if(isNaN(row)){
						var rows   = JSON.parse('["'+row.replace(/,/g, '","')+'"]');
					...
					$(this).attr('data-row-nummer', ii+'.');
					
					if($.inArray(i+1,result) > -1){
										
						$(this).addClass('pp-highlight');
